...
Public Class frmRecordCourse
    Private connectionString As String = ConnectionStr

    Private Sub frmRecordCourse_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        'Populate the course combobox with data from tblStudents
        Dim query As String = "SELECT DISTINCT [Course] FROM tblStudents;"
        Using connection As New OleDbConnection(connectionString)
            ...
            For Each row As DataRow In data.Rows
                cmbSections.Items.Add(row("Course").ToString())
            Next
        End Using
    End Sub
    ...
    Private Sub cmbSections_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbSections.SelectedIndexChanged
        'Load the students in the selected course to the datagridview
        Dim courseName As String = cmbSections.SelectedItem.ToString()
        Dim query As String = "SELECT * FROM tblStudents WHERE [Course]=?"
        Using connection As New OleDbConnection(connectionString)
            connection.Open()
            Dim adapter As New OleDbDataAdapter(query, connection)
            adapter.SelectCommand.Parameters.AddWithValue("?", courseName)
            Dim data As New DataTable()
            adapter.Fill(data)
